/* Shared script: navbar, footer, loader, language, animations, sounds, theme */
...
(function() {
  ...
  // Inject Navbar and Footer (keeps consistent across pages)
  function injectNavbar() {
        ...
        <div class="nav-left">
          <a class="nav-logo" href="index.html?lang=${lang}" aria-label="Home">
            <svg viewBox="0 0 64 64" aria-hidden="true">
              <circle cx="32" cy="32" r="18" fill="#FFD400" stroke="black" stroke-width="2"/>
              <path d="M44,22 C48,16 54,16 58,22 C54,24 48,24 44,22 Z" fill="#78C257" stroke="black" stroke-width="1.5"/>
            </svg>
        ...
        <div class="nav-right">
          <a class="social-icon" href="https://instagram.com" target="_blank" rel="noopener" aria-label="Instagram">
            ...
            <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#fff" stroke-width="1.5">
              <rect x="2" y="2" width="20" height="20" rx="5" stroke="#fff"/>
              <circle cx="12" cy="12" r="5" stroke="#fff"/>
          ...
          <a class="social-icon" href="https://tiktok.com" target="_blank" rel="noopener" aria-label="TikTok">
            <!-- TikTok logo -->
            <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="#fff" stroke-width="1.5">
              <path d="M14 3v8.5a4.5 4.5 0 1 1-3.9-4.46" stroke="#fff"/>
              <path d="M14 6c1.2 1.7 3.2 3 5 3" stroke="#fff"/>
            </svg>
          </a>
    ...
    // Nav hide on scroll (down hides, up shows)
    ...
    // Lang change across pages
    document.getElementById('langSelect').addEventListener('change', (e) => {
      setLang(e.target.value);
      // Update links to carry ?lang
  ...
  function applyTranslations(lang) {
    ...
    if (document.title && /LIMONI WEB/i.test(document.title)) {
      // keep main title name; no change
    }
  }
  ...
  const applyTheme = () => {
    ...
    else if (saved === 'light') body.classList.add('theme-light');
    else body.classList.add(prefDark ? 'theme-dark' : 'theme-light');
  };
  ...
  // Sound effects with WebAudio (no external files)
  ...
  // Buttons behaviors (particles, explode, sparkle) + navigation
  ...
  // Navigation helper with lang preserved
  function navigate(key) {
    const lang = getLang();
    let target = 'index.html';
    if (key === 'inizia') {
      // Opens a page with centered text and same background as home per request: use about-like card
      target = 'iscriviti.html'; // The user requested a dedicated "Inizia" page text; mapping to iscriviti.html not correct?
      // Better create a dedicated page? We'll route to about-like text in this repository:
      target = 'inizia.html';
    }
    if (key === 'israel') target = 'israel.html';
    if (key === 'palestina') target = 'palestina.html';
    if (key === 'iscriviti') target = 'iscriviti.html';
    location.href = `${target}?lang=${lang}`;
  }

  // Create an optional "inizia.html" on the fly warning if missing
  // For static GH Pages, we actually included a physical file below in instructions.
  ...
  // Ensure "Torna indietro" links preserve lang
